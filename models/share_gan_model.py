...
class ShareGANModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.loss_names = ['D_A', 'G', 'G_A', 'cycle_A', 'idt_A', 'D_B', 'G_B', 'cycle_B', 'idt_B']
        ...
        self.netG = networks3D.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.norm,
                                        not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids, opt.usejoint)
        ...
        if self.isTrain:
            ...
            self.criterionGAN = GANLoss(gpu_ids=self.gpu_ids, use_lsgan=not opt.no_lsgan).cuda()
    ...
    def set_input(self, input):
        ...
        self.real_A = input[0 if AtoB else 1].cuda()
        self.real_B = input[1 if AtoB else 0].cuda()
    ...
    def backward_D_B(self, update_step=1.):
    ...
    def backward_G(self, update_step=1.):
        ...
        # combined loss
        self.loss_G = (
                                  self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B + self.loss_idt_A + self.loss_idt_B) / update_step
